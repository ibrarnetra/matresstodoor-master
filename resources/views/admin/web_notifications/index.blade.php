                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Web-Notifications')
                            <a href="{{route('web-notifications.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('web-notifications.bulkDelete')}}')"><i
                            ...
                            <table id="generic-datatable" class="table table-striped table-sm gy-2 gs-2 align-middle">
                                <thead>
                                    <tr class="fw-bolder fs-6 text-gray-800">
                                        ...
                                        <th style="width: 65%;">Notification</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$web_notification->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{substr(strip_tags($web_notification->notification),0,100)."..."}}</td>
                                        <td>{{$web_notification->sort_order}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$web_notification->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$web_notification->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('web-notifications.update-status', ['id' => $web_notification->id])}}', '{{$web_notification->status}}', false)">
                                                {{$web_notification->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Web-Notifications')
                                            <a href="{{route('web-notifications.edit', ['id' => $web_notification->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            {{-- @can('Delete-Web-Notifications')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('web-notifications.delete', ['id' => $web_notification->id])}}', false)" data-bs-toggle="tooltip"
