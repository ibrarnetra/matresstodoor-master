        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('customer-groups.store')}}" @else
                        action="{{route('customer-groups.update', ['id' => $id])}}" @endif>
                        <div class="card-body pt-0">
                            @csrf
                            <input type="hidden" name="type" id="type" value="{{$type}}">
                            <div class="row">
                                <div class="mb-5 col-md-6">
                                    <label class="form-label">Approve New Customers </label>
                                    <div class="form-check form-check-solid form-check-inline">
                                        <input class="form-check-input" type="radio" id="yes" name="approval" value="1" @if($type=="create" ) checked @endif @if($type=="edit" &&
                                            $modal['approval']=="1" ) checked @endif />
                                        <label class="form-check-label" for="yes">
                                            Yes
                                        </label>
                                    </div>
                                    <div class="form-check form-check-solid form-check-inline">
                                        <input class="form-check-input" type="radio" id="no" name="approval" value="0" @if($type=="edit" && $modal['approval']=="0" ) checked @endif />
                                ...
                                <div class="mb-5 col-md-6">
                                    ...
                                    <input type="number" min="1" class="form-control form-control-solid @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" @if($type=="create"
                                        ) value="{{ old('sort_order') }}" @else value="{{ old('sort_order') ?: $modal['sort_order'] }}" @endif>
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane" id="{{$language->code}}" role="tabpanel">
                                    <div class="row mt-8">
                                        <div class="mb-5 col-md-6">
                                            <label class="form-label required" for="customer_group[{{$language->code}}][name]">Name</label>
                                            <input type="text" class="form-control form-control-solid @error('customer_group.'.$language->code.'.name') is-invalid @enderror"
                                                id="customer_group[{{$language->code}}][name]" name="customer_group[{{$language->code}}][name]" required @if($type=="create" )
                                                value="{{ old('customer_group.'.$language->code.'.name') }}" @else
                                                value="{{ old('customer_group.'.$language->code.'.name') ?: $modal['customer_group'][''.$language->code.'']['name'] }}" @endif>

                                            @error('customer_group.'.$language->code.'.name')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                            @enderror
                                        ...
                                        <div class="mb-5 col-md-6">
                                            <label for="customer_group[{{$language->code}}][description]" class="form-label required">Description </label>
                                            <textarea class="form-control form-control-solid @error('customer_group.'.$language->code.'.description') is-invalid @enderror"
                                                name="customer_group[{{$language->code}}][description]" id="customer_group[{{$language->code}}][description]" rows="3"
                                                required>@if($type == "create"){{ old('customer_group.'.$language->code.'.description') }}@else{{ old('customer_group.'.$language->code.'.description') ?: $modal['customer_group'][''.$language->code.'']['description'] }}@endif</textarea>

                                            @error('customer_group.'.$language->code.'.description')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                            @enderror
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('customer-groups.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
