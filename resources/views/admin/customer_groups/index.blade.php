                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Customer-Groups')
                            <a href="{{route('customer-groups.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('customer-groups.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                                Delete</a>
                        </div>
                    </div>
                    <!--end::Header-->
                            ...
                            <table id="generic-datatable" class="table table-striped table-sm gy-2 gs-2 align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <!--begin::Checkbox-->
                                        <td>
                                            @if ($customer_group->is_editable == getConstant('IS_EDITABLE'))
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$customer_group->id}}" />
                                            </div>
                                            @endif
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$customer_group->eng_description->name}}</td>
                                        <td>{{$customer_group->sort_order}}</td>
                                        <td>
                                            @if ($customer_group->is_editable == getConstant('IS_EDITABLE'))
                                            <a href="javascript:void(0);" class="badge badge-light-{{$customer_group->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$customer_group->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('customer-groups.update-status', ['id' => $customer_group->id])}}', '{{$customer_group->status}}', false)">
                                                {{$customer_group->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                            @endif
                                        </td>
                                        <td>
                                            @can('Edit-Customer-Groups')
                                            <a href="{{route('customer-groups.edit', ['id' => $customer_group->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            @can('Delete-Customer-Groups')
                                            @if ($customer_group->is_editable == getConstant('IS_EDITABLE'))
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('customer-groups.delete', ['id' => $customer_group->id])}}', false)" data-bs-toggle="tooltip"
