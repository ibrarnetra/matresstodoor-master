                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Pages')
                            <a href="{{route('pages.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('pages.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                            Delete</a> --}}
                        </div>
                    </div>
                    <!--end::Header-->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$page->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$page->eng_description->title}}</td>
                                        <td>{{substr(strip_tags($page->eng_description->content),0,100)."..."}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$page->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$page->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('pages.update-status', ['id' => $page->id])}}', '{{$page->status}}', false)">
                                                {{$page->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Pages')
                                            <a href="{{route('pages.edit', ['id' => $page->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                            ...
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('pages.delete', ['id' => $page->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
