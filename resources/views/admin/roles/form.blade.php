        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" @if($type=="create" ) action="{{route('roles.store')}}" @else action="{{route('roles.update', ['id' => $id])}}" @endif>
                        <div class="card-body pt-0">
                            @csrf
                            <div class="row">
                                <div class="col-md-4 mb-5">
                                    <label for="name" class="required form-label">Role Name</label>
                                    <input type="text" class="form-control form-control-solid @error('name') is-invalid @enderror" id="name" name="name" placeholder="Role Name" autocomplete="off"
                                        required @if($type=="create" ) value="{{ old('name') }}" @else value="{{ old('name') ?: $modal->name}}" @endif />
                                    @error('name')
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-5">
                                    <label for="" class="required form-label">Permissions</label>
                                    @error('permissions')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 mb-4">
                                            <div class="form-check form-switch form-check-custom form-check-solid">
                                                <input class="form-check-input" type="checkbox" name="permissions[]" value="{{$permission->name}}" id="permissions-{{$permission->id}}"
                                                    @if($type=="edit" ) @if(in_array($permission->id,$old_permission_arr))
                                                checked
                                                @endif
                                                @endif/>
