                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Stock-Locations')
                            <a href="{{route('store-locations.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('store-locations.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                                Delete</a>
                        </div>
                    </div>
                    <!--end::Header-->
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="table-responsive">
                            <!--begin::Table-->
                            <table id="generic-datatable" class="table table-striped table-sm gy-5 gs-7">
                                <thead>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$store->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$store->name}}</td>
                                        <td>{{$store->address}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$store->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$store->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('store-locations.update-status', ['id' => $store->id])}}', '{{$store->status}}', false)">
                                                {{$store->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Store-Locations')
                                            <a href="{{route('store-locations.edit', ['id' => $store->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            @can('Delete-Store-Locations')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('store-locations.delete', ['id' => $store->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
