                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Tax-Rates')
                            <a href="{{route('tax-rates.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('tax-rates.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                            Delete</a> --}}
                        </div>
                    </div>
                    <!--end::Header-->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$tax_rate->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$tax_rate->name}}</td>
                                        <td>{{$tax_rate->rate}}</td>
                                        <td>{{ucfirst($tax_rate->type)}}</td>
                                        <td>
                                            @if ($tax_rate->geozone)
                                            {{$tax_rate->geozone->name}}
                                            @else
                                            N/A
                                            @endif
                                        </td>
                                        <td>{{date("Y-m-d", strtotime($tax_rate->created_at))}}</td>
                                        <td>{{date("Y-m-d", strtotime($tax_rate->updated_at))}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$tax_rate->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$tax_rate->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('tax-rates.update-status', ['id' => $tax_rate->id])}}', '{{$tax_rate->status}}', false)">
                                                {{$tax_rate->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Tax-Rates')
                                            <a href="{{route('tax-rates.edit', ['id' => $tax_rate->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                            ...
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('tax-rates.delete', ['id' => $tax_rate->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
