        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('tax-rates.store')}}" @else action="{{route('tax-rates.update', ['id' => $id])}}" @endif>
                        <div class="card-body pt-0">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-5 col-md-6">
                                    ...
                                    <input type="number" step="0.01" min="0" class="form-control form-control-solid @error('rate') is-invalid @enderror" id="rate" name="rate" required
                                        @if($type=="create" ) value="{{ old('rate') }}" @else value="{{ old('rate') ?: $modal['rate'] }}" @endif>
                            ...
                            <div class="row">
                                <div class="mb-5 col-md-6">
                                    ...
                                    <select class="form-select form-select-solid" aria-label="type" id="type" name="type">
                                ...
                                <div class="mb-5 col-md-6">
                                    <label class="form-label " for="geo_zone_id">Geo Zone</label>
                                    <select class="form-select form-select-solid geozone" aria-label="geo_zone_id" id="geo_zone_id" name="geo_zone_id">
                                        <option value="" disabled selected>-- Select GeoZone --</option>
                                        @if (count($geozones))
                                        @foreach ($geozones as $geozone)
                                        <option value="{{$geozone->id}}" @if ($type=='edit' && $modal['geo_zone_id']==$geozone->id)
                                            selected
                                            @endif>{{$geozone->name}}</option>
                                        @endforeach
                                        @endif
                                    </select>
                                    @error('geo_zone_id')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 mb-4">
                                            <div class="form-check form-switch form-check-custom form-check-solid">
                                                <input class="form-check-input" type="checkbox" name="customer_groups[]" value="{{$customer_group->id}}" id="customer_groups-{{$customer_group->id}}"
                                                    @if($type=="edit" ) @if(in_array($customer_group->id,$modal['customer_groups']))
                                                checked
                                                @endif
                                                @endif />
                                                <label class="form-check-label" for="customer_groups-{{$customer_group->id}}">
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('tax-rates.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
