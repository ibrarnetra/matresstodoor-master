                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="row row-sample d-none">
                        ...
                        <div class="col-md-5 mb-5">
                            <input type="number" min="1" name="sort_order" class="form-control form-control-solid" value="1" required>
                        </div>
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('sliders.store')}}" @else action="{{route('sliders.update', ['id' => $id])}}" @endif>
                        <div class="card-body pt-0">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-5 col-md-6">
                                    <label class="form-label required" for="sort_order">Sort Order</label>
                                    <input type="number" min="1" class="form-control form-control-solid @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" required
                            ...
                            <div id="item_container">
                                ...
                                @if($type == "edit")
                                @if(count($modal['slides']) > 0)
                                @foreach($modal['slides'] as $key => $value)
                                <script>
                                    temp_index ='{{$key + 1}}';
                                </script>
                                <div class="row">
                                    <input type="hidden" name="slides[{{$key}}][id]" value="{{$value->id}}">
                                    <div class="col-md-4 mb-5">
                                        @if ($value->image)
                                        <input type="hidden" name="slides[{{$key}}][old_image]" value="{{$value->image}}">
                                        @endif
                                        <input type="file" name="slides[{{$key}}][image]" class="form-control form-control-solid" accept="image/*">
                                    </div>
                                    <div class="col-md-2 mb-5">
                                        @if ($value->image)
                                        <img class="img-fluid" width="35" height="35" src="{{asset('storage/slider_images/150x150/'.$value->image)}}" alt="{{$value->image}}">
                                        @else
                                        N/A
                                        @endif
                                    </div>
                                    <div class="col-md-5 mb-5">
                                        <input type="number" min="1" name="slides[{{$key}}][sort_order]" value="{{$value->sort_order}}" class="form-control form-control-solid">
                                    </div>
                                    <div class="col-md-1 mb-5 text-end">
                                        <button class="btn btn-sm btn-danger del_button" type="button" data-url="{{route('sliders.deleteSlide', ['id' => $value->id])}}"><i
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-5 text-end">
                                    <button class="btn btn-primary btn-sm btn-rounded" type="button" onclick="addNewRow(['image', 'sort_order'])">
...
@push('page_lvl_js')
<script src="{{asset('/')}}custom/sliders.js"></script>
@endpush