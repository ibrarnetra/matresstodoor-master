                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Options')
                            <a href="{{route('options.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('options.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                                Delete</a>
                        </div>
                    </div>
                    <!--end::Header-->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$option->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$option->eng_description->name}}</td>
                                        <td>{{$option->sort_order}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$option->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$option->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('options.update-status', ['id' => $option->id])}}', '{{$option->status}}', false)">
                                                {{$option->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Options')
                                            <a href="{{route('options.edit', ['id' => $option->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                            ...
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('options.delete', ['id' => $option->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
