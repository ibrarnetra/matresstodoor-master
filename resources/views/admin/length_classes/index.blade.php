                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Length-Classes')
                            <a href="{{route('length-classes.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('length-classes.bulkDelete')}}')"><i
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$length_class->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$length_class->eng_description->title}}</td>
                                        <td>{{$length_class->eng_description->unit}}</td>
                                        <td>{{$length_class->value}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$length_class->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$length_class->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('length-classes.update-status', ['id' => $length_class->id])}}', '{{$length_class->status}}', false)">
                                                {{$length_class->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Length-Classes')
                                            <a href="{{route('length-classes.edit', ['id' => $length_class->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            {{-- @can('Delete-Length-Classes')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('length-classes.delete', ['id' => $length_class->id])}}', false)" data-bs-toggle="tooltip"
