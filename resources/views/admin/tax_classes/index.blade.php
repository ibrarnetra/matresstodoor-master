                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Tax-Classes')
                            <a href="{{route('tax-classes.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('tax-classes.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                            Delete</a> --}}
                        </div>
                    </div>
                    <!--end::Header-->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$tax_class->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        ...
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$tax_class->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$tax_class->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('tax-classes.update-status', ['id' => $tax_class->id])}}', '{{$tax_class->status}}', false)">
                                                {{$tax_class->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Tax-Classes')
                                            <a href="{{route('tax-classes.edit', ['id' => $tax_class->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            {{-- @can('Delete-Tax-Classes')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('tax-classes.delete', ['id' => $tax_class->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
