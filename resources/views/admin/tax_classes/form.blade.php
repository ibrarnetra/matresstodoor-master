                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="row row-sample d-none">
                        <div class="col-md-4 mb-5">
                            <select type="text" id="tax_rate_id" class="form-select form-select-solid" required>
                        ...
                        <div class="col-md-4 mb-5">
                            <select type="text" id="based" class="form-select form-select-solid" required>
                        ...
                        <div class="col-md-3 mb-5">
                            <input type="number" min="1" id="priority" class="form-control form-control-solid" required>
                        </div>
                    ...
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('tax-classes.store')}}" @else action="{{route('tax-classes.update', ['id' => $id])}}"
                        @endif>
                        <div class="card-body pt-0">
                            ...
                            <div class="row">
                                <div class="mb-5 col-md-6">
                                    ...
                                    <input type="text" class="form-control form-control-solid @error('title') is-invalid @enderror" id="title" name="title" required @if($type=="create" )
                                        value="{{ old('title') }}" @else value="{{ old('title') ?: $modal['title'] }}" @endif>
                                ...
                                <div class="mb-5 col-md-6">
                                    ...
                                    <textarea required class="form-control form-control-solid @error('description') is-invalid @enderror" name="description" id="description"
                                        rows="3">@if($type == "create"){{ old('description') }}@else{{ old('description') ?: $modal['description'] }}@endif</textarea>
                            ...
                            <div class="multiple @if($type == " edit") d-none @endif" id="item_container">
                                <hr>
                                <div class="row">
                                    <div class="col-md-4">
                                ...
                                @if($type == "edit")
                                @if(count($modal['tax_rules']) > 0)
                                @foreach($modal['tax_rules'] as $k => $v)
                                <div class="row row-{{$k}}" data-id="{{$k}}">
                                    <div class="col-md-4 mb-5">
                                        <select type="text" name="tax_rule[{{$k}}][tax_rate_id]" class="form-select form-select-solid" required>
                                            @foreach ($tax_rates as $tax_rate)
                                            <option value="{{$tax_rate->id}}" @if ($v->tax_rate_id == $tax_rate->id)
                                                selected
                                                @endif>{{$tax_rate->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 mb-5">
                                        <select type="text" name="tax_rule[{{$k}}][based]" class="form-select form-select-solid" required>
                                            <option value="shipping" @if ($v->based == "shipping")
                                                selected
                                                @endif>Shipping Address</option>
                                            <option value="payment" @if ($v->based == "payment")
                                                selected
                                                @endif>Payment Address</option>
                                            <option value="store" @if ($v->based == "store")
                                                selected
                                                @endif>Store Address</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3 mb-5">
                                        <input type="number" min="1" name="tax_rule[{{$k}}][priority]" value="{{$v->priority}}" class="form-control form-control-solid" required>
                                    </div>
                                    <div class="col-md-1 mb-5 text-end">
                                        <button class="btn btn-danger btn-sm del_button" type="button"><i class="fas fa-minus-circle"></i>
                                        </button>
                                    </div>
                                </div>
                                @endforeach
                            ...
                            <div class="row multiple @if($type == " edit") d-none @endif">
                                <div class="col-md-12 mb-5 text-end">
                                    <button class="btn btn-primary btn-sm btn-rounded add_new_button" type="button" onclick="addNewRow()">
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('tax-classes.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
...
@push('page_lvl_js')
<script src="{{asset('/')}}custom/taxClass.js"></script>
@endpush