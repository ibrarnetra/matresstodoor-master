            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <!--end::Header-->
                        <!--begin::Body-->
                        <form method="POST" enctype="multipart/form-data" @if ($type == 'create') action="{{ route('routes.store') }}" @else action="{{ route('routes.update', ['id' => $id]) }}" @endif>
                            <div class="card-body pt-0">
                                ...
                                <div class="row mt-8">
                                    <div class="mb-5 col-md-6">
                                        ...
                                        <input type="text"
                                            ...
                                            id="name" name="name" @if ($type == 'create') value="{{ old('name') }}"
                                    @else value="{{ old('name') ?: $modal['name'] }}" @endif
                                        required>
                                    ...
                                    <div class="mb-5 col-md-6">
                                        ...
                                        <select
                                            ...
                                            @foreach ($stores as $store)
                                                <option value="{{ $store->id }}" @if ($type == 'edit' && $modal['start_location_id'] == $store->id) selected @endif>
                                                    {{ $store->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row">
                                    <div class="mb-5 col-md-8">
                                        ...
                                        <input
                                            ...
                                            name="end_location" id="end_location" value="{{ $modal['end_location'] }}">
                                        @error('end_location')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                    ...
                                    <div class="mb-5 col-md-4">
                                        ...
                                        <div class="input-group">
                                            <input class="form-control form-control-solid" name="dispatch_date"
                                                autocomplete="off" id="dispatch_date" placeholder="Pick date">
                                ...
                                <div class="row">
                                    <div class="mb-5 col-md-6">
                                        ...
                                        <select
                                            class="form-select form-select-solid select2 order-id @error('order_id') is-invalid @enderror"
                                            name="order_id" id="order_id" style="width: 100%;">
                                        </select>
                                        @error('order_id')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="table-responsive">
                                            <table
                                                class="table table-sm table-row-bordered table-striped gy-2 gs-2 align-middle">
                                                <thead>
                                                ...
                                                <tbody id="order-list">
                                                    @if (count($modal['route_locations']) > 0)
                                                        @foreach ($modal['route_locations'] as $location)
                                                            @if ($location->order)
                                                                <tr>
                                                                    <td>
                                                                        {{ $location->order->id }}
                                                                        <input class="id" type="hidden"
                                                                            name="orders[{{ $location->order->id }}][id]"
                                                                            value="{{ $location->order->id }}"
                                                                            data-order-status="{{ $location->order->order_status->name }}">
                                                                    </td>
                                                                    <td>{{ $location->order->first_name . ' ' . $location->order->last_name }}
                                                                    </td>
                                                                    <td>
                                                                        {{ $location->order->shipping_address_1 }}
                                                                        <input class="shipping_address_1" type="hidden"
                                                                            name="orders[{{ $location->order->id }}][shipping_address_1]"
                                                                            value="{{ $location->order->shipping_address_1 }}">
                                                                    </td>

                                                                    <td>{{ $location->order->order_status->name }}
                                                                    </td>
                                                                    <td>
                                                                        <input type="number"
                                                                            name="orders[{{ $location->order->id }}][sort_order]"
                                                                            class="form-control form-control-sm form-control-solid sort_order"
                                                                            placeholder="Sort Order" autocomplete="off"
                                                                            value="{{ $location->sort_order }}">
                                                                        <input type="hidden"
                                                                            name="orders[{{ $location->order->id }}][order_status_id]"
                                                                            class="form-control form-control-sm form-control-solid"
                                                                            autocomplete="off"
                                                                            value="{{ isset($location->route_order_status) ? $location->route_order_status->id : $location->order->order_status->id }}">
                                                                    </td>
                                                                    <td>
                                                    ...
                                                    @else
                                                        <tr>
                                                            <td colspan="6" class="text-center"><strong>No data
                                                                    found...</strong></td>
                                                        </tr>
                                                    @endif
                                                </tbody>
                            ...
                            <div class="card-footer">
                                <div class="row">
                                    <div class="col-md-12 text-start">
                                        ...
                                        <a class="btn btn-secondary" href="{{ route('routes.index') }}">Cancel</a>
                                    </div>
                                </div>
                            </div>
                        </form>
...
@push('page_lvl_js')
    ...
    <script>
        $(document).ready(function() {
            initSelect2Ajax($("#order_id"), '{{ route('orders.search') }}');
            let dispatchDate = new Date();
            @if (isset($modal['dispatch_date']) && !is_null($modal['dispatch_date']) && $modal['dispatch_date'] != '')
            dispatchDate = '{{ $modal['dispatch_date'] }}';
            @endif
        ...
        $("#order_id").on("select2:select", function(e) {
            ...
            $("#order-list").append(data.tr);
            $('table tr:last').find('.sort_order').val(rowCount);
        });
