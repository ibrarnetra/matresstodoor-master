            ...
            <div class="card shadow">
               <div class="card-header border-1 mb-5 pb-3 pt-5">
                  <h3 class="card-title align-items-start flex-column">
                     <span class="card-label fw-bolder fs-3 mb-1"><i class="fas fa-route"></i> &nbsp;{{$route->name}} Cash Summary</span>
                  </h3>
                  ...
                  <div class="row">
                     <div class="col-md-12">
                        <div class="table-responsive-lg">
                           <table  class="table table-sm gy-4 gs-4">
                              <thead>
                                 @php 
                                 $total_paid_amount = 0;
                              ...
                              @endphp
                                 ...
                                 <tr class="fw-bolder fs-6 text-gray-800">
                                    ...
                                    <th  style="width: 15%">Collected Amount</th>
                        ...
                        */
                        ...
                        $grand_remaining_total += $remaining_amount = ($payment_exists) ? setDefaultPriceFormat($remaining_amount) : $total_amount;
                                 ...
                                 <tr class="{{($location->order->custom_order == 'No' || $location->order->order_status_id == '16' || $location->order->order_status_id == '20') ?$location->order->order_status->color_class:'special_order_color'}}">
                                    ...
                                    <td>
                                     ...
                                     <td class="text-center">${{setDefaultPriceFormat($location->order->total)}}</td>
                                    <td class="text-center">{{isset($location->order->total_collect_amount)?'$'.setDefaultPriceFormat($location->order->total_collect_amount):'$'.setDefaultPriceFormat("0")}}</td>
                                  
                                    @php 
                                       $total_paid_amount += $location->order->total_collect_amount;
                                    @endphp   
                                   
                                    <!--begin::Action=-->
                                    <td class="text-start"  colspan="3">
                                       <a  href="javascript:void(0);" @if($location->order->order_status->color_class) style="color:white" @elseif($location->order->custom_order == 'Yes') style="color:white" @endif
                                       onclick="routeOrderCashSummary(this, '{{$location->order->id}}', '{{route('routes.routeOrderCashSummary', ['order_id' => $location->order->id, 'route_id' => $route->id])}}')">Cash Detail</a>
                                          ...
                                          <div class="menu-item px-3">
                                             <a href="javascript:void(0);" class="menu-link px-3" onclick="deleteData('{{route('route-locations.delete', ['id' => $location->id])}}', false)">
                                                Delete
                                             </a>
                                          </div>

                                          <div class="menu-item px-3">
                                             <a class="menu-link px-3" href="javascript:void(0);" id="copy-address" onclick="copySingleAddress(this)">Copy Address</a>
                                          </div> --}}
                              ...
                              <tfoot>
                                 <tr>
                                    <td colspan="5" class="text-end fw-bolder fs-6 text-gray-800 p-3">Collected Cash Amount</td>
                                    <td  class="text-end fw-bolder fs-6 text-gray-800 p-3">${{setDefaultPriceFormat($total_cash_amount)}}</td>
                                 </tr>
                                 <tr>
                                    <td colspan="5" class="text-end fw-bolder fs-6 text-gray-800 p-3">Online Transfer Amount</td>
                                    <td  class="text-end fw-bolder fs-6 text-gray-800 p-3">${{setDefaultPriceFormat($total_online_amount)}}</td>
                                 </tr>
                                 <tr>
                                    <td colspan="5" class="text-end fw-bolder fs-6 text-gray-800 p-3">Credit/Debit Amount</td>
                                    <td  class="text-end fw-bolder fs-6 text-gray-800 p-3">${{setDefaultPriceFormat($total_card_amount)}}</td>
                                 </tr>
                                
                                 <tr>
                                    <td colspan="5" class="text-end fw-bolder fs-6 text-gray-800 p-5">Total Collected Amount</td>
                                    <td  class="text-end fw-bolder fs-6 text-gray-800 p-5">${{setDefaultPriceFormat($total_paid_amount)}}</td>
                                 </tr>
                                 <tr>
                                    <td colspan="5" class="text-end fw-bolder fs-6 text-gray-800 p-3">Total Remaining Amount</td>
                                    <td  class="text-end fw-bolder fs-6 text-gray-800 p-3">${{setDefaultPriceFormat($grand_remaining_total)}}</td>
                                 </tr>
                              </tfoot>
                           </table>
                        </div>
      ...
      <div class="modal-dialog modal-xl">
         <div class="modal-content">
            <div class="modal-header">
               <h2 class="modal-title"><i class="fas fa-sync-alt"></i> &nbsp;Route Order Cash Detail</h2>
