            ...
            <div class="row mt-5">
                <div class="col-md-12">
                    <div class="card shadow">
                        <div class="card-header border-1 mb-2 pb-2 pt-5">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder fs-3 mb-1"><i class="fas fa-boxes"></i>
                                    &nbsp;Items</span>
                            </h3>
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 py-7 menu-state-bg-light-primary fw-bold w-300px"
                                data-kt-menu="true">
                                    @can('Add-Loading-Sheets')
                                    <div class="menu-item px-3">
                                        <a  class="menu-link px-3" data-bs-toggle="modal"
                                            data-bs-target="#add-item">Add new item</button>
                                    </a>
                                    </div>

                                    @endcan
                                    <div class="menu-item px-3">
                                    <a class="menu-link px-3" href="{{ route('orders.exportLoadingSheet', ['loading_sheet_id' => $loading_sheet_id]) }}"
                                       > Export Loading
                                        Sheet</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a  class="menu-link px-3" href="{{ route('loading-sheets.manual-add-loading-sheet', ['loading_sheet_id' => $loading_sheet_id]) }}"
                                            >Manual Add Loading
                                            Sheet</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a class="menu-link px-3" href="{{ route('routes.truck-loaded', ['id' => $route_id]) }}">
                                            Loading Done</a>
                                    </div>
                                @endif
                            </div>
                                        ...
                                        <table
                                            ...
                                            <thead>
                                                <tr class="fw-bolder fs-6 text-gray-800">
                                                    ...
                                                    <th style="width: 40%;" class="text-center">Quantity</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @if (count($item_all) > 0)
                                                    ...
                                                    @foreach ($item_all as $loading_sheet_item)
                                                        ...
                                                        <tr>
                                                            <td>
                                                                {!! $loading_sheet_item['product_name'] !!}
                                                            </td>
                                                            <td class="text-center">
                                                                {{ $loading_sheet_item['quantity'] }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                @else
                                                    <tr>
                                                        <td colspan="5" class="text-center"><strong>No Data
                                                                Found...</strong></td>
                                                    </tr>
                                                @endif
                                            </tbody>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">Add Item to Loading Sheet
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('loading-sheets.addItem') }}"
                        onsubmit="return addItem(this, event.preventDefault());">
                        @csrf
                        <input type="hidden" id="inventory_from" name="inventory_from" value="warehouse">
                        <input type="hidden" id="loading_sheet_id" name="loading_sheet_id"
                            value="{{ $loading_sheet_id }}">
                        <div class="row">
                            <div class="mb-5 col-md-6">
                                <label class="form-label" for="">Choose Product </label>
                                <select class="form-select from-select-solid product @error('product') is-invalid @enderror"
                                    name="product" id="product" style="width: 100%;" required>
                                </select>
                            </div>
                            <div class="mb-5 col-md-6">
                                <label class="form-label" for="qty">Quantity</label>
                                <input type="number" min="1" value="1"
                                    class="form-control form-control-solid @error('qty') is-invalid @enderror" name="qty"
                                    id="qty" required>
                            </div>
                        </div>
...
@push('page_lvl_js')
    <script src="{{ asset('/') }}custom/loadingSheet.js"></script>
@endpush
