                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Stock-Statuses')
                            <a href="{{route('stock-statuses.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('stock-statuses.bulkDelete')}}')"><i
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$stock_status->id}}" />
                                            </div>
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$stock_status->name}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="badge badge-light-{{$stock_status->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$stock_status->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('stock-statuses.update-status', ['id' => $stock_status->id])}}', '{{$stock_status->status}}', false)">
                                                {{$stock_status->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                        </td>
                                        <td>
                                            @can('Edit-Stock-Statuses')
                                            <a href="{{route('stock-statuses.edit', ['id' => $stock_status->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            {{-- @can('Delete-Stock-Statuses')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('stock-statuses.delete', ['id' => $stock_status->id])}}', false)" data-bs-toggle="tooltip"
