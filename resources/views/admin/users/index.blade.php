                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Users')
                            <a href="{{route('users.create')}}" class="btn btn-sm btn-primary"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                        </div>
                    </div>
                    <!--end::Header-->
                                ...
                                <table id="generic-datatable" class="table table-striped table-sm gy-2 gs-2 align-middle">
                                    <thead>
                                        <tr class="fw-bolder fs-6 text-gray-800">
                                            ...
                                            <th style="width: 7%">Permissions</th>
...
<script>
    ...
    let configOptions = {
        ...
        Auth::guard('web')->user()->hasRole("Office Admin"))
        { 
            data: "textual_password", 
            name: "textual_password", 
            orderable: false, 
            searchable: false
        },
        @endif
    ...
    function assignUnassignTeamLead(dis,url){
        let team_lead_id = $("option:selected", dis).val();
        $.ajax({
            type: "POST",
            url: url,
            data: {
            ...
            success: function (res) {
                console.log("🚀 ~ file: index.blade.php ~ line 230 ~ assignUnassignTeamLead ~ res", res)
            },
            error: function (err) {
                console.log("🚀 ~ file: index.blade.php ~ line 234 ~ assignUnassignTeamLead ~ err", err)
            }
        });
    }

    $(document).on("click",".multi-assign-checkbox", function () {
        let toggle = false;
        setTimeout(() => {
            ...
            if (toggle) {
                $("#multi-assign").removeClass("d-none");
            } else {
                $("#multi-assign").addClass("d-none");
            }
        }, 100);
    });
