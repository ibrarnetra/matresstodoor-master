        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-1 mb-5 pb-3 pt-5">
                ...
                <div class="card-toolbar">
                    ...
                    <a href="{{route('manufacturers.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                    @endcan
                    <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('manufacturers.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                        Delete</a>
                </div>
            </div>
            <!--end::Header-->
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                                        <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$manufacturer->id}}" />
                                    </div>
                                </td>
                                <!--end::Checkbox-->
                                <td>
                                    @if ($manufacturer->image)
                                    <img class="img-fluid" width="35" height="35" src="{{asset('storage/manufacturer_images/'.$manufacturer->image)}}" alt="{{$manufacturer->image}}">
                                    @else
                                    N/A
                                    @endif
                                </td>
                                ...
                                <td>
                                    <a href="javascript:void(0);" class="badge badge-light-{{$manufacturer->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                        data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$manufacturer->status == " 1" ? "Active" : "Inactive"}}"
                                        onclick="updateStatus('{{route('manufacturers.update-status', ['id' => $manufacturer->id])}}', '{{$manufacturer->status}}', false)">
                                        {{$manufacturer->status == "1" ? "Active" : "Inactive"}}
                                    </a>
                                </td>
                                <td>
                                    <div class="dropdown">
                                        <button class="btn btn-light btn-active-light-primary btn-sm dropdown-toggle" type="button" id="{{$manufacturer->id}}" data-bs-toggle="dropdown"
                                            aria-expanded="false">
                                            Action
                                        </button>
                                        <ul class="dropdown-menu" aria-labelledby="{{$manufacturer->id}}">
                                            @can('Edit-Manufacturers')
                                            <a href="{{route('manufacturers.edit', ['id' => $manufacturer->id])}}" class="dropdown-item">
                                                Edit
                                            </a>
                                            @endcan

                                            <a href="javascript:void(0);" class="dropdown-item" onclick="toggleSlugModal('{{route('manufacturers.editSlug', ['id' => $manufacturer->id])}}')">
                                                Edit Slug
                                            </a>

                                            @can('Delete-Manufacturers')
                                            <a href="javascript:void(0);" class="dropdown-item" onclick="deleteData('{{route('manufacturers.delete', ['id' => $manufacturer->id])}}', false)">
                                                Delete
                                            </a>
                                            @endcan
                                        </ul>
