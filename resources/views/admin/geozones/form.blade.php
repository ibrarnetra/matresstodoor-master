                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="row row-sample d-none">
                        <div class="col-md-6 mb-5">
                            <select id="country" class="form-select form-select-solid select2-country" onchange="getZones(this, '{{route('zones.getZones')}}')" style="width: 100%;">
                        ...
                        <div class="col-md-5 mb-5">
                            <select id="zone" class="form-select form-select-solid select2-zone select2-country" style="width: 100%;">
                    ...
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('geozones.store')}}" @else action="{{route('geozones.update', ['id' => $id])}}" @endif>
                        <div class="card-body pt-0">
                            ...
                            <div class="row">
                                <div class="mb-5 col-md-6">
                                    ...
                                    <input type="text" class="form-control form-control-solid @error('name') is-invalid @enderror" id="name" name="name" required @if($type=="create" )
                                        value="{{ old('name') }}" @else value="{{ old('name') ?: $modal['name'] }}" @endif>
                            ...
                            <div class="multiple @if($type == " edit") d-none @endif" id="item_container">
                                ...
                                @if($type == "edit")
                                @if(count($modal['countries']) > 0)
                                @foreach($modal['countries'] as $k => $v)
                                <div class="row row-{{$k}}" data-id="{{$k}}">
                                    <div class="col-md-6 mb-5">
                                        <select name="geozone[{{$k}}][country]" class="form-select form-select-solid country" onchange="getZones(this, '{{route('zones.getZones')}}')"
                                            style="width: 100%">
                                            <option value="" disabled selected>-- Select Country --</option>
                                            @if (count($countries) > 0)
                                            @foreach ($countries as $country)
                                            <option value="{{$country->id}}" @if ($country->id == $v->id)
                                                selected
                                                @endif>{!! $country->name !!}</option>
                                            @endforeach
                                            @endif
                                    ...
                                    <div class="col-md-5 mb-5">
                                        <select name="geozone[{{$k}}][zone]" class="form-select form-select-solid zone" style="width: 100%">
                                            <option value="" disabled selected>-- Select Zone --</option>
                                            @foreach ($v->zones as $zone)
                                            <option value="{{$zone->id}}" @if ($zone->id == $modal['zones'][$k]['id'])
                                                selected
                                                @endif>{!! $zone->name !!}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-1 mb-5 text-end">
                                        <button class="btn btn-danger del_button" type="button"><i class="fas fa-minus-circle"></i>
                                        </button>
                                    </div>
                                </div>
                                @endforeach
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('geozones.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
...
@push('page_lvl_js')
<script src="{{asset('/')}}custom/geoZone.js"></script>
@endpush