        ...
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}"
            ...
            <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">{{$title}}
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <li class="breadcrumb-item text-muted">Attribute Manager</li>
                    <li class="breadcrumb-item">
                        <span class="bullet bg-gray-200 w-5px h-2px"></span>
                    </li>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('attributes.store')}}" @else action="{{route('attributes.update', ['id' => $id])}}" @endif>
                            ...
                            <div class="tab-content tabcontent-border mt-3">
                                <div class="row">
                                    <div class="mb-5 col-md-6">
                                        <label for="attribute_group_id" class="form-label required">Attribute Group</label>
                                        <select class="form-select form-select-solid" aria-label="attribute_group_id" id="attribute_group_id" name="attribute_group_id" required>
                                            @if(count($attribute_groups) > 0)
                                            @foreach($attribute_groups as $attribute_group)
                                            <option value="{{$attribute_group->id}}" @if(isset($modal) && $modal['attribute_group_id']==$attribute_group->id)
                                                selected
                                                @endif>
                                                {{$attribute_group->eng_description->name}}
                                            </option>
                                            @endforeach
                                            @endif
                                        </select>
                                ...
                                <div class="tab-pane active" id="general" role="tabpanel">
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div class="tab-pane" id="{{$language->code}}" role="tabpanel">
                                            <div class="row mt-8">
                                                <div class="mb-5 col-md-6">
                                                    <label class="form-label required" for="attribute_description[{{$language->code}}][name]">Name</label>
                                                    <input type="text" class="form-control form-control-solid @error('attribute_description.'.$language->code.'.name') is-invalid @enderror"
                                                        id="attribute_description[{{$language->code}}][name]" name="attribute_description[{{$language->code}}][name]" required @if($type=="create" )
                                                        value="{{ old('attribute_description.'.$language->code.'.name') }}" @else
                                                        value="{{ old('attribute_description.'.$language->code.'.name') ?: $modal['attribute_description'][''.$language->code.'']['name'] }}" @endif>

                                                    @error('attribute_description.'.$language->code.'.name')
                                                    <div class="invalid-feedback">
                                                        {{ $message }}
                                                    </div>
                                                    @enderror
