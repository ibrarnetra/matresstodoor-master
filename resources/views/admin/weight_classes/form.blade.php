        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('weight-classes.store')}}" @else action="{{route('weight-classes.update', ['id' => $id])}}"
                        @endif>
                                ...
                                <div class="tab-pane" id="{{$language->code}}" role="tabpanel">
                                    <div class="row mt-8">
                                        <div class="mb-5 col-md-6">
                                            <label class="form-label required" for="weight_class_description[{{$language->code}}][title]">Title</label>
                                            <input type="text" maxlength="32" class="form-control form-control-solid @error('weight_class_description.'.$language->code.'.title') is-invalid @enderror"
                                                id="weight_class_description[{{$language->code}}][title]" name="weight_class_description[{{$language->code}}][title]" required @if($type=="create" )
                                                value="{{ old('weight_class_description.'.$language->code.'.title') }}" @else
                                                value="{{ old('weight_class_description.'.$language->code.'.title') ?: $modal['weight_class_description'][''.$language->code.'']['title'] }}" @endif>

                                            @error('weight_class_description.'.$language->code.'.title')
                                        ...
                                        <div class="mb-5 col-md-6">
                                            <label class="form-label required" for="weight_class_description[{{$language->code}}][unit]">Unit</label>
                                            <input type="text" maxlength="4" class="form-control form-control-solid @error('weight_class_description.'.$language->code.'.unit') is-invalid @enderror"
                                                id="weight_class_description[{{$language->code}}][unit]" name="weight_class_description[{{$language->code}}][unit]" required @if($type=="create" )
                                                value="{{ old('weight_class_description.'.$language->code.'.unit') }}" @else
                                                value="{{ old('weight_class_description.'.$language->code.'.unit') ?: $modal['weight_class_description'][''.$language->code.'']['unit'] }}" @endif>

                                            @error('weight_class_description.'.$language->code.'.unit')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                            @enderror
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('weight-classes.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
