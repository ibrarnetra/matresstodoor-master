        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('faqs.store')}}" @else action="{{route('faqs.update', ['id' => $id])}}" @endif>
                                    ...
                                    <div class="row">
                                        <div class="mb-5 col-md-4">
                                            ...
                                            <input type="number" min="1" class="form-control form-control-solid @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order"
                                                @if($type=="create" ) value="{{ old('sort_order') ?: "1" }}" @else value="{{ old('sort_order') ?: $modal['sort_order'] }}" @endif>
                                        ...
                                        <div class="tab-pane" id="{{$language->code}}" role="tabpanel">
                                            <div class="row mt-8">
                                                <div class="mb-5 col-md-12">
                                                    <label class="form-label required" for="faq_description[{{$language->code}}][question]">Question</label>
                                                    <textarea class="form-control form-control-solid @error('faq_description.'.$language->code.'.question') is-invalid @enderror"
                                                        name="faq_description[{{$language->code}}][question]" placeholder="Question" id="faq_description[{{$language->code}}][question]"
                                                        rows="3">@if($type == "create"){{ old('faq_description.'.$language->code.'.question') }}@else{{ old('faq_description.'.$language->code.'.question') ?: $modal['faq_description'][''.$language->code.'']['question'] }}@endif</textarea>

                                                    @error('faq_description.'.$language->code.'.question')
                                                    <div class="invalid-feedback">
                                            ...
                                            <div class="row">
                                                <div class="mb-5 col-md-12">
                                                    <label class="form-label required" for="faq_description[{{$language->code}}][answer]">Answer</label>
                                                    <textarea class="form-control form-control-solid @error('faq_description.'.$language->code.'.answer') is-invalid @enderror"
                                                        name="faq_description[{{$language->code}}][answer]" placeholder="Answer" id="faq_description[{{$language->code}}][answer]"
                                                        rows="3">@if($type == "create"){{ old('faq_description.'.$language->code.'.answer') }}@else{{ old('faq_description.'.$language->code.'.answer') ?: $modal['faq_description'][''.$language->code.'']['answer'] }}@endif</textarea>

                                                    @error('faq_description.'.$language->code.'.answer')
                                                    <div class="invalid-feedback">
                                                        <strong>{{ $message }}</strong>
                                                    </div>
                                                    @enderror
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('faqs.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
...
@push('page_lvl_js')
<script src="{{asset('/')}}custom/faqs.js"></script>
@endpush