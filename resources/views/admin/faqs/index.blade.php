                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Faqs')
                            <a href="{{route('faqs.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            {{-- <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('faqs.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                            Delete</a> --}}
                        </div>
                    </div>
                    <!--end::Header-->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$faq->id}}" />
                        </div>
                        </td>
                        <!--end::Checkbox--> --}}
                        <td>{{substr(strip_tags($faq->eng_description->question),0,100)."..."}}</td>
                        <td>{{substr(strip_tags($faq->eng_description->answer),0,100)."..."}}</td>
                        <td>
                            <a href="javascript:void(0);" class="badge badge-light-{{$faq->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
                                data-bs-placement="top" title="{{$faq->status == " 1" ? "Active" : "Inactive"}}"
                                onclick="updateStatus('{{route('faqs.update-status', ['id' => $faq->id])}}', '{{$faq->status}}', false)">
                                {{$faq->status == "1" ? "Active" : "Inactive"}}
                            </a>
                        </td>
                        <td>
                            @can('Edit-Faqs')
                            <a href="{{route('faqs.edit', ['id' => $faq->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                            ...
                            {{-- @can('Delete-Faqs')
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('faqs.delete', ['id' => $faq->id])}}', false)" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark"
