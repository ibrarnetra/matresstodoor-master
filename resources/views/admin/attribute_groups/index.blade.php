                ...
                <div class="card mb-5 mb-xl-8">
                    ...
                    <div class="card-header border-1 mb-5 pb-3 pt-5">
                        ...
                        <div class="card-toolbar">
                            @can('Add-Attribute-Groups')
                            <a href="{{route('attribute-groups.create')}}" class="btn btn-sm btn-primary create"><i class="fas fa-plus-circle"></i> Create New</a>
                            @endcan
                            <a href="javascript:void(0);" class="btn btn-sm btn-danger delete d-none" onclick="bulkDelete('{{route('attribute-groups.bulkDelete')}}')"><i class="fas fa-trash-alt"></i>
                                Delete</a>
                        </div>
                    </div>
                    <!--end::Header-->
                            ...
                            <table id="generic-datatable" class="table table-striped table-sm gy-2 gs-2 align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <!--begin::Checkbox-->
                                        <td>
                                            @if ($attribute->is_editable == getConstant('IS_EDITABLE'))
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input delete-checkbox" type="checkbox" name="id" value="{{$attribute->id}}" />
                                            </div>
                                            @endif
                                        </td>
                                        <!--end::Checkbox-->
                                        <td>{{$attribute->eng_description->name}}</td>
                                        <td>{{$attribute->sort_order}}</td>
                                        <td>
                                            @if ($attribute->is_editable == getConstant('IS_EDITABLE'))
                                            <a href="javascript:void(0);" class="badge badge-light-{{$attribute->status == " 1" ? "success" : "danger"}}" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="{{$attribute->status == " 1" ? "Active" : "Inactive"}}"
                                                onclick="updateStatus('{{route('attribute-groups.update-status', ['id' => $attribute->id])}}', '{{$attribute->status}}', false)">
                                                {{$attribute->status == "1" ? "Active" : "Inactive"}}
                                            </a>
                                            @endif
                                        </td>
                                        <td>
                                            @can('Edit-Attribute-Groups')
                                            <a href="{{route('attribute-groups.edit', ['id' => $attribute->id])}}" class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip"
                                                data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan

                                            @can('Delete-Attribute-Groups')
                                            @if ($attribute->is_editable == getConstant('IS_EDITABLE'))
                                            <a href="javascript:void(0);" class="btn btn-icon btn-active-light-primary w-30px h-30px"
                                                onclick="deleteData('{{route('attribute-groups.delete', ['id' => $attribute->id])}}', false)" data-bs-toggle="tooltip"
