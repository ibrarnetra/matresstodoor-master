        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-5 mb-xl-8">
                    ...
                    <!--end::Header-->
                    <!--begin::Body-->
                    <form method="POST" enctype="multipart/form-data" @if($type=="create" ) action="{{route('attribute-groups.store')}}" @else
                        action="{{route('attribute-groups.update', ['id' => $id])}}" @endif>
                                ...
                                <div class="tab-pane active" id="general" role="tabpanel">
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div class="tab-pane" id="{{$language->code}}" role="tabpanel">
                                            <div class="row mt-8">
                                                <div class="mb-5 col-md-6">
                                                    <label class="form-label required" for="attribute_group_description[{{$language->code}}][name]">Name</label>
                                                    <input type="text" class="form-control form-control-solid @error('attribute_group_description.'.$language->code.'.name') is-invalid @enderror"
                                                        id="attribute_group_description[{{$language->code}}][name]" name="attribute_group_description[{{$language->code}}][name]" required
                                                        @if($type=="create" ) value="{{ old('attribute_group_description.'.$language->code.'.name') }}" @else
                                                        value="{{ old('attribute_group_description.'.$language->code.'.name') ?: $modal['attribute_group_description'][''.$language->code.'']['name'] }}"
                                                        @endif>

                                                    @error('attribute_group_description.'.$language->code.'.name')
                                                    <div class="invalid-feedback">
                                                        {{ $message }}
                                                    </div>
                                                    @enderror
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12 text-start">
                                    ...
                                    <a class="btn btn-secondary" href="{{route('attribute-groups.index')}}">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </form>
