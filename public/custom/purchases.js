...
function submitFrom() {
    ...
    let unit_price = $("#purchase_unit_price").val();
    let quantity = $("#product_qty").val();
    let warehouse_id =
    $(".warehouse option:selected").val() == undefined
        ? 0
        : $(".warehouse option:selected").val();
    // if( unit_price === undefined || unit_price == null || unit_price == NaN || unit_price<1)
    // {
    ...
    if (product_id == undefined || product_id == null || product_id == NaN) {
        ...
        if (processing) {
            ...
            $.ajax({
                ...
                dataType: "JSON",
                success: function (res) {
                    $("#purchase-cart-item").html(res.data);
...
function delRow(dis, url, cart_id) {
    ...
    $.ajax({
        ...
        data: {
            id: cart_id,
            warehouse_id: sanitizeVal($("#warehouse_id")),
        },
        dataType: "JSON",
        success: function (res) {
...
function generateCartForEdit(purchase_id) {
    let currency_symbol = $("#currency_id option:selected").attr("data-symbol");

    $.ajax({
        type: "GET",
        url: BASE_URL + "purchases/generate-cart-for-edit",
        data: {
...
$("#input-discount-amount").on("change focusout", function () {
    calGrandTotal();
    recalculateRemainingAmount();
});
...
function handleOrderSubmit(dis) {
    ...
    if (!valid) {
        if (paymentType === "partial") {
            toastr.warning("The calculated bills grand total is not equal to order paid amount!");
        } else {
            toastr.warning("The calculated bills grand total is not equal to order grand total!");
        }
    }
    return valid;
}
